//% color=190 weight=100 icon="\uf1ec" block="RADLX16A" advanced=true
namespace RADLX16A {
    let initialized: boolean = false
    let packet: number[] = []
    let servos: number[] = []

    //% blockid=RADLX16A_initialize
    //% block="RADLX16A!initialize"
    export function initialize(): void {
        let txport: number = 8
        let rxport: number = 12
        let baudrate: number = 115200
        serial.redirect(txport, rxport, baudrate)
        initialized = true
    }
    ...
    function checkSum(inpacket: number[]): number {
        let s: number = 0
        let l: number = inpacket.length
        while (l > 2) {
            l = l - 1
            s = s + packet[l]
        }
    ...
    function toBytes(n: number): number[] {
        let numberArrayResult: number[] = []
        if ((n < 0) || (n > 65535)) {
            n = 0
        }
        numberArrayResult[0] = n && 255
        numberArrayResult[1] = n // 256
        return numberArrayResult
    }

    function sendPacket(inpacket: number[]): void {
        inpacket[inpacket.length] = checkSum(inpacket)
        let outBuffer: Buffer = pins.createBufferFromArray(inpacket)
        serial.writeBuffer(outBuffer)
    }

    function isPacketGood(inpacket: number[]): boolean {
        let booleanResult: boolean = false
        let s: number = 0
        let l: number = inpacket.length
        l = l - 1
        ...
        s = ~s
        s = s & 255
        if (s == inpacket[inpacket.length - 1]) {
            booleanResult = true
        }
        return booleanResult
    }

    //% blockid=RADLX16A_moveTimeWrite
    //% block="RADLX16A!moveTimeWrite"
    export function moveTimeWrite(servoid: number, angle: number, time: number = 0): void {
        let servoAngle: number = Math.floor(angle * 25 / 6)
        packet = []
        packet[0] = 0x55
        packet[1] = 0x55
        packet[2] = servoid
        packet[3] = 7
        packet[4] = 1
        let angleBytes: number[] = toBytes(angle)
        packet[5] = angleBytes[0]
        packet[6] = angleBytes[1]
        let timeBytes: number[] = toBytes(time)
        packet[7] = timeBytes[0]
        packet[8] = timeBytes[1]
        sendPacket(packet)
    }
